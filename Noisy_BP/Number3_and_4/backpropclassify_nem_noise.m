...
for epoch = 1:maxepoch
    ...
    for batch = 1
        ...
        for idx = 2:1:num1-2
            ...
            nn.hsamp{idx} = zeros(N,L(idx+1)+1,maxMC);
        end;
        ...
        for iterMC=1:maxMC
            ...
            Targetout_samp(:,:,iterMC) = Targetout_samp(:,:,iterMC)./repmat(row_sum(Targetout_samp(:,:,iterMC)),1,L(num1));
        ...
        %% Add noise to output layer based on the hyperplane condition
        nv = noisevar/(epoch^annfactor);
        noise = nv*randn(N,L(num1));
        criterion = zeros(N,1);
        criterion_rev = zeros(N,1);
        for iterMC=1:maxMC
            criterion = criterion + Sampimp(:,iterMC).*row_sum(noise.*log(Targetout_samp(:,:,iterMC)));
        end
        mask = (criterion >= 0);
        targets = targets + noise.*repmat(mask,1,L(num1));
